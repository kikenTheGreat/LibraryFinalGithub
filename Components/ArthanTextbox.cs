...
namespace LibraryCGC.Components
{
    ...
    public partial class ArthanTextBox : UserControl
    {
        #region -> Fields
        ...
        //-> Constructor
        public ArthanTextBox()
        {
            InitializeComponent();

            // allow free resizing
            textBox1.Multiline = true;       // enable multiline so height can grow
            textBox1.Dock = DockStyle.Fill;  // make textbox follow the control size
        }
        ...
        protected override void WndProc(ref Message m)
        {
            ...
            if (m.Msg == WM_NCHITTEST)
            {
                ...
                if ((int)m.Result == HTCLIENT)
                {
                    ...
                    int grip = 8; // Grip size

                    if (cursor.X <= grip && cursor.Y <= grip) m.Result = (IntPtr)HTTOPLEFT;
                    else if (cursor.X >= this.Width - grip && cursor.Y <= grip) m.Result = (IntPtr)HTTOPRIGHT;
                    else if (cursor.X <= grip && cursor.Y >= this.Height - grip) m.Result = (IntPtr)HTBOTTOMLEFT;
                    else if (cursor.X >= this.Width - grip && cursor.Y >= this.Height - grip) m.Result = (IntPtr)HTBOTTOMRIGHT;
                    else if (cursor.X <= grip) m.Result = (IntPtr)HTLEFT;
                    else if (cursor.X >= this.Width - grip) m.Result = (IntPtr)HTRIGHT;
                    else if (cursor.Y <= grip) m.Result = (IntPtr)HTTOP;
                    else if (cursor.Y >= this.Height - grip) m.Result = (IntPtr)HTBOTTOM;
        ...
        {
            // only force height if single line
